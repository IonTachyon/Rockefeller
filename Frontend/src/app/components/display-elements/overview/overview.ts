      ...
      this.trades.forEach(trade => {
        ...
        if (ticker) {
            ...
            if(pieEntry === undefined) {
              pieEntry = {tickerID: ticker.id, symbol: ticker.symbol, name: ticker.name, value: ticker.price * trade.volume, average_purchase_price: trade.price, total_purchased: 0, total_sold: 0};
              if(trade.volume > 0)
              {
